...
#Total 27260 records exported to CRMLSListing202602.csv
import csv
import requests
...
if token:
    ...
    params = {
        ...
        '$filter': f"ListingContractDate ge {datetime(2024, 2, 1).isoformat(timespec='milliseconds')}Z and ListingContractDate lt {datetime(2024, 3, 1).isoformat(timespec='milliseconds')}Z",        '$top': 1000  # Extracting up to 1000 observations
    }

    # Send a GET request to the API endpoint with the token and parameters
    total_records = 0
    csv_file = 'csv/CRMLSListing202402.csv'
